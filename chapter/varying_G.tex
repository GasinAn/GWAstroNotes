...
\end{equation}
设待估参数为$\mu=(\ln\xi,\ln (d_\text{L}/{d_\text{L}}_0),\ln Q,\dots)$, $\dots$为其他参数(如$\mathcal{M}$), 则$\tilde{h}_{,\ln\xi}=\tilde{h}_{,\ln (d_\text{L}/{d_\text{L}}_0)}=-\tilde{h}_{,\ln Q}=-\tilde{h}$, $\tilde{h}$对其他参数求偏导皆为纯虚数, 则由$\Gamma_{ab}=\left\langle h_{,a}\mid h_{,b}\right\rangle $和$\text{SNR}:=\rho=\sqrt{\left\langle h\mid h\right\rangle}$得
\begin{equation}
...
\cite{Abbott2021,Thrane2019}, For a given beyond-GR parameter $x$, this distribution $p(x\mid d)$ is the expectation for $x$ after marginalizing over the hyperparameters $\mu$ and $\sigma$,
\begin{equation}
    p(x\mid d)=\int p(x\mid \mu,\sigma)p(\mu,\sigma\mid d)\,\d\mu\d\sigma,
\end{equation}
where $d$ represents the data for \emph{all} detected events, and $p(x\mid \mu,\sigma)\sim\mathcal{N}(\mu,\sigma)$ by construction.

$p(\mu,\sigma\mid d)$算法见 \verb\mid hp.py\mid .
