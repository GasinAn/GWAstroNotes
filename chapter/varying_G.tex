...
If $G=G(r)$, for $h(r,t)=h(r)e^{-i\omega t}=h(r)e^{-i2\pi ft}$, $h(r)e^{-i2\pi ft}$ and $h(r)$ will satisfy some linear differential equations known. For $h(r,t)=\int\tilde{h}(r;f)e^{-i2\pi ft}\,df$, If $\tilde{h}(r;f)e^{-i2\pi ft}$ and $\tilde{h}(r;f)$ satisfy those equations, their linear combination $h(r,t)$ will also satisfy those differential equations. therefore $\tilde{h}(r;f)\leftrightarrow h(r)$.
